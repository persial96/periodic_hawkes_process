...
data {
  ...
  //
  // use_seasonality = 1:
  //     proposed annual sine-cosine baseline
  //
  // use_seasonality = 0:
  //     constant baseline benchmark
...
parameters {
  ...
  //
  // This means that the benchmark does not estimate unnecessary
  // seasonal parameters.
...
model {
  ...
  to_vector(alpha) ~ beta(
    alpha_prior_a,
    alpha_prior_b
  );

  to_vector(beta) ~ gamma(
    beta_prior_shape,
    beta_prior_rate
  );
